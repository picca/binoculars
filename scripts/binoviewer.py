...
#the values corresponds to the pixel center. ie, there is 11 points in the [0,1] range with 0.1 resolution 
#the data are loaded in the form of a numpy array of shape (nx,ny,nz)
...
from guidata.qt.QtGui import (qApp,QCheckBox,QVBoxLayout,QHBoxLayout,QGridLayout,QLabel,QLineEdit,QKeyEvent,QWidget,QSlider,
                              QSpinBox,QDoubleSpinBox,QColor,QComboBox,QAction,QMenu,QMenuBar,QProgressBar,QInputDialog,QPolygonF,
                              QPushButton,QButtonGroup,QDialog,QMessageBox,QToolBar,QGroupBox)
...
from guiqwt.signals import (SIG_ITEMS_CHANGED,SIG_STOP_NOT_MOVING,SIG_MOVE,SIG_STOP_MOVING,SIG_TOOL_JOB_FINISHED,
                            SIG_START_TRACKING,SIG_MARKER_CHANGED,SIG_RANGE_CHANGED,SIG_ITEM_SELECTION_CHANGED,
                            SIG_VALIDATE_TOOL,SIG_ACTIVE_ITEM_CHANGED)
from guiqwt.styles import ImageParam,MaskedImageParam,CurveParam
from guiqwt.tools import (OpenFileTool,CommandTool,DefaultToolbarID,InteractiveTool,
                          RectangleTool,CircleTool,FreeFormTool,
                          BasePlotMenuTool,AntiAliasingTool,SelectTool)
from guiqwt.panels import PanelWidget
from guiqwt.shapes import XRangeSelection,Marker,PointShape,RectangleShape,EllipseShape,PolygonShape 
import tables
import numpy as np
from guiqwt._scaler import _histogram
from scipy.interpolate import griddata
from scipy.ndimage import laplace,measurements,maximum,uniform_filter
...
class LaplaceParam(DataSet):
    ...
    fill = FloatItem("starting values",default=1.0,min=0.)
    fillc = FloatItem("contributions",default=10,min=1.).set_pos(col=1)
...
class MaskedImageNan(MaskedImageItem):
    ...
    def mask_square_area(self, x0, y0, x1, y1, inside=True,
                              trace=True, do_signal=True):
        """
        ...
        if inside:
            ...
            self.data[indexes] = np.ma.masked
        if trace:
            self.add_masked_area('square', np.array([[x0, y0],[x1, y0],[x1, y1],[x0, y1]]), inside, mask=True)
        if do_signal:
            self._mask_changed()
    ...
    def unmask_rectangular_area(self, x0, y0, x1, y1, inside=True,
                              trace=True, do_signal=True):
        """
        ...
        if trace:
            self.add_masked_area('rectangular', np.array([[x0, y0],[x1, y0],[x1, y1],[x0, y1]]), inside, mask=False)
        if do_signal:
            self._mask_changed()

    def unmask_square_area(self, x0, y0, x1, y1, inside=True,
                              trace=True, do_signal=True):
        """
        Unmask square area
        If inside is True (default), unmask the inside of the area
        ...
        if inside:
            ...
            self.data.mask[indexes] = False
        if trace:
            self.add_masked_area('square', np.array([[x0, y0],[x1, y0],[x1, y1],[x0, y1]]), inside, mask=False)
        if do_signal:
            self._mask_changed()

    def unmask_circular_area(self, x0, y0, x1, y1, inside=True,
                           trace=True, do_signal=True):
        """
        ...
        if trace:
            ...
            dx=abs(x1-x0)/2.
            dy=abs(y1-y0)/2.
            self.add_masked_area('circular', np.array([[xc, yc+dy],[xc, yc-dy],[xc+dx, yc],[xc-dx, yc]]), inside, mask=False)
        if do_signal:
            self._mask_changed()
                        
    def unmask_polygonal_area(self, pts, inside=True,
                           trace=True, do_signal=True):
        """
...
class PlotItemBuilder(GuiPlotItemBuilder):
    ...
    def binocurve(self, x, y, title=u"",
...
class SlicePrefs():
    ...
    labels=['x','y','z']
    ranges=[1.,1.,1.]
    pos1=[0.,0.,0.]  #position en bas de tige
    pos2=[0.,0.,1.]  #position en haut de tige
...
class MoveHandler(DragHandler):
    def __init__(self, filter, btn, mods=Qt.NoModifier, start_state=0):
        super(MoveHandler, self).__init__(filter, btn, mods,
                                                          start_state)
...
class DrawingTool(InteractiveTool):
    ...
    def __init__(self, manager, handle_final_shape_cb=None, shape_style=None,
        ...
        super(DrawingTool, self).__init__(manager, toolbar_id,
...
class RectangleEraserTool(RectangleDrawingTool):
    ...
    def __init__(self, manager, handle_final_shape_cb=None, shape_style=None,
                 toolbar_id=DefaultToolbarID, title=None, icon=None, tip=None,
                 switch_to_default_tool=None):
        super(RectangleEraserTool, self).__init__(manager,  toolbar_id=toolbar_id,
                                title=title, icon=icon, tip=tip,
                                switch_to_default_tool=switch_to_default_tool)
        self.handle_final_shape_cb = handle_final_shape_cb
        self.shape = None
...
class CircularMaskTool(CircularDrawingTool):
    ...
    def __init__(self, manager, handle_final_shape_cb=None, shape_style=None,
                 toolbar_id=DefaultToolbarID, title=None, icon=None, tip=None,
                 switch_to_default_tool=None):
        super(CircularMaskTool, self).__init__(manager,  toolbar_id=toolbar_id,
                                title=title, icon=icon, tip=tip,
                                switch_to_default_tool=switch_to_default_tool)
        self.handle_final_shape_cb = handle_final_shape_cb
        self.shape = None
...
class EllipseMaskTool(EllipseDrawingTool):
    ...
    def __init__(self, manager, handle_final_shape_cb=None, shape_style=None,
                 toolbar_id=DefaultToolbarID, title=None, icon=None, tip=None,
                 switch_to_default_tool=None):
        super(EllipseMaskTool, self).__init__(manager,  toolbar_id=toolbar_id,
                                title=title, icon=icon, tip=tip,
                                switch_to_default_tool=switch_to_default_tool)
        self.handle_final_shape_cb = handle_final_shape_cb
        self.shape = None
...
class RectangleMaskTool(RectangleDrawingTool):
    ...
    def __init__(self, manager, handle_final_shape_cb=None, shape_style=None,
                 toolbar_id=DefaultToolbarID, title=None, icon=None, tip=None,
                 switch_to_default_tool=None):
        super(RectangleMaskTool, self).__init__(manager,  toolbar_id=toolbar_id,
                                title=title, icon=icon, tip=tip,
                                switch_to_default_tool=switch_to_default_tool)
        self.handle_final_shape_cb = handle_final_shape_cb
        self.shape = None
...
class SetSliceWindow(QDialog):
    ...
    def __init__(self,prefs):
        self.prefs=prefs
        super(SetSliceWindow, self).__init__()  #permet l'initialisation de la fenetre sans perdre les fonctions associees
        ...
        stepn=int((prefs.maxs[prefs.i1]-prefs.mins[prefs.i1])/prefs.steps[prefs.i1])
        if stepn==0:
            stepn=1
        ...
        QObject.connect(self.stepn,SIGNAL("valueChanged(int)"),self.compute_step_width)
        QObject.connect(self.stepw,SIGNAL("textEdited(QString)"),self.compute_step_number)   #only when user change the text!
    ...
    def appl (self):   
        ...
        try:
            ...
            self.prefs.stepw=(self.prefs.maxs[self.prefs.i1]-self.prefs.mins[self.prefs.i1])/self.prefs.stepn
...
class Set2DSliceWindow(QDialog):
    ...
    def __init__(self,prefs):
        self.prefs=prefs
        super(Set2DSliceWindow, self).__init__()  #permet l'initialisation de la fenetre sans perdre les fonctions associees
        self.setWindowTitle("Parameters for slice generation")
        self.setFixedSize(QSize(450, 160))
        self.layout=QGridLayout()
        self.setLayout(self.layout)
...
class SetSliceWindow2(QDialog):
    ...
    def __init__(self,prefs):
        self.prefs=prefs
        super(SetSliceWindow2, self).__init__()  #permet l'initialisation de la fenetre sans perdre les fonctions associees
        self.setWindowTitle("Parameters for slice generation")
        #self.setFixedSize(QSize(450, 160))
        self.layout=QGridLayout()
        self.setLayout(self.layout)
        ...
        QObject.connect(self.Cancel, SIGNAL("clicked()"), self.closewin)
        QObject.connect(self.OK, SIGNAL("clicked()"), self.appl)
        QObject.connect(self.combo11, SIGNAL("activated(int)"),self.set_slice_direction)
        ...
        QObject.connect(self.box12,SIGNAL("textEdited(QString)"),self.set_steps)   #only when user change the text!
    ...
    def set_slice_direction(self,i2):
        ...
        stepmax=int((self.prefs.absmaxs[i2]-self.prefs.absmins[i2])/self.prefs.steps[i2])
        self.box15.setMaximum(stepmax)
        step=int((self.prefs.absmaxs[i2]-self.prefs.absmins[i2])/self.prefs.steps[i2])
        self.box15.setValue(step)
...
class DoubleSpinSliderBox(QWidget):
  ...
  def __init__ (self, parent = None):
    ...
    self.connect(self.slider, SIG_INT_VALUE_CHANGED, self.update_from_slider)
    self.connect(self.slider, SIG_SLIDER_PRESSED, lambda:self.emit(SIG_SLIDER_PRESSED))
    self.connect(self.slider, SIG_SLIDER_RELEASED, lambda:self.emit(SIG_SLIDER_RELEASED))
...
def couple_doublespinsliders(spsl1,spsl2):
    QObject.connect(spsl1,SIG_DOUBLE_VALUE_CHANGED,lambda x:spsl2.setValue(max(x,spsl2.value())))
    QObject.connect(spsl2,SIG_DOUBLE_VALUE_CHANGED,lambda x:spsl1.setValue(min(x,spsl1.value())))
...
class ImageMaskingWidget(PanelWidget):
    ...
    def configure_panel(self):
        """Configure panel"""
        self.ellipse_mask_tool  = self.manager.add_tool(EllipseMaskTool,toolbar_id="masking shapes")
        self.rect_mask_tool  = self.manager.add_tool(RectangleMaskTool,toolbar_id="masking shapes")
        ...
        self.ellipse_tool = self.manager.add_tool(CircleTool, toolbar_id="masking shapes",
                                     handle_final_shape_cb=lambda shape:
                                       self.handle_shape(shape),
    ...
    def setup_actions(self):
        ...
        QObject.connect(self.minspinslider, SIG_DOUBLE_VALUE_CHANGED, self.update_threshold)
        QObject.connect(self.maxspinslider, SIG_DOUBLE_VALUE_CHANGED, self.update_threshold)
    ...
    def update_threshold(self,x):
        ...
        vmin=np.percentile(self.masked_image.data[np.isfinite(self.masked_image.data)],pmin)
        vmax=np.percentile(self.masked_image.data[np.isfinite(self.masked_image.data)],pmax)
    ...
    def mask_dilatation(self):
        ...
        self.masked_image.data.mask=signal.fftconvolve(self.masked_image.data.mask,struct,'same')>0.5  #much better than ndimage.binary_dilatation
    ...
    def mask_erosion(self):
        ...
        self.masked_image.data.mask=signal.fftconvolve(np.logical_not(self.masked_image.data.mask),struct,'same')<=0.5  #much better than ndimage.binary_dilatation
    ...
    def apply_shape_mask(self, shape, inside, mask):
        ...
        if isinstance(shape, RectangleShape):
            ...
            if mask:
                ...
                self.masked_image.unmask_rectangular_area(x0, y0, x1, y1,
                                                        inside=inside)                                                    
            ...
            if mask:              
                ...
                self.masked_image.unmask_circular_area(x0, y0, x1, y1,
                                                     inside=inside)
        elif isinstance(shape, PolygonShape):
            if mask:              
                self.masked_image.mask_polygonal_area(shape.points,inside=inside)
                text="adding masked polygonal area to"+self.masked_image.imageparam.label
                for pt in shape.points:
                    text+="(%f,%f),"%(pt[0],pt[1])
                text+=" inside="+str(inside)
            else:
                self.masked_image.unmask_polygonal_area(shape.points,inside=inside)
                text="adding unmasked polygonal area to"+self.masked_image.imageparam.label
                for pt in shape.points:
                    text+="(%f,%f),"%(pt[0],pt[1])
                text+=" inside="+str(inside)
...
class ProjectionWidget(PanelWidget):
    ...
    def __init__(self,parent=None):
        ...
        self.plane=QCheckBox('plane selection',self)  #select a specific plane instead of a range
        ...
        self.xbgrange1 = BgRangeSelection(0,1)  #range selection for background subtraction
        self.ybgrange1  =BgRangeSelection(0,1) 
        self.zbgrange1  =BgRangeSelection(0,1)

        self.xbgrange2 = BgRangeSelection(0,1)  #range selection for background subtraction
        ...
        self.xmarker = Marker(constraint_cb=lambda x,y:self.xcurve.get_closest_x(x, y))
        self.xmarker.set_markerstyle('|')
        self.ymarker = Marker(constraint_cb=lambda x,y:self.ycurve.get_closest_x(x, y))
        self.ymarker.set_markerstyle('|')
        self.zmarker = Marker(constraint_cb=lambda x,y:self.zcurve.get_closest_x(x, y))
    ...
    def setup_connect(self):
        ...
        QObject.connect(self.show_contributions,SIGNAL('clicked()'),self.other_changed)
    ...
    def update_bg_range(self):        
        ...
        dxs2=self.spacings[self.axis]/2.
        if self.axis==0:
    ...
    def move_xcursor(self,i):
        if self.plane.isChecked():
            ...
            self.xrange.set_range(_min+i*self.spacings[0],_max+i*self.spacings[0])
    ...
    def move_ycursor(self,i):
        if self.plane.isChecked():
            ...
            self.yrange.set_range(_min+i*self.spacings[1],_max+i*self.spacings[1])
    ...
    def move_zcursor(self,i):
        if self.plane.isChecked():
            ...
            self.zrange.set_range(_min+i*self.spacings[2],_max+i*self.spacings[2])
    ...
    def set_ranges(self,ranges,spacings=None,dosignal=False):
        ...
        #selection
        self.xrange.set_range(ranges[0][0]-self.spacings[0]/2.,ranges[0][1]+self.spacings[0]/2.,dosignal=dosignal)
        self.yrange.set_range(ranges[1][0]-self.spacings[1]/2.,ranges[1][1]+self.spacings[1]/2.,dosignal=dosignal)
        self.zrange.set_range(ranges[2][0]-self.spacings[2]/2.,ranges[2][1]+self.spacings[2]/2.,dosignal=dosignal)
        
        #background left
        self.xbgrange1.set_range(ranges[0][0]-3.*self.spacings[0]/2.,ranges[0][0]-self.spacings[0]/2.,dosignal=dosignal)
        self.ybgrange1.set_range(ranges[1][0]-3.*self.spacings[1]/2.,ranges[1][0]-self.spacings[1]/2.,dosignal=dosignal)
        self.zbgrange1.set_range(ranges[2][0]-3.*self.spacings[2]/2.,ranges[2][0]-self.spacings[2]/2.,dosignal=dosignal)
        
        #background right
        self.xbgrange2.set_range(ranges[0][1]+self.spacings[0]/2.,ranges[0][1]+3.*self.spacings[0]/2.,dosignal=dosignal)
        self.ybgrange2.set_range(ranges[1][1]+self.spacings[1]/2.,ranges[1][1]+3.*self.spacings[1]/2.,dosignal=dosignal)
        self.zbgrange2.set_range(ranges[2][1]+self.spacings[2]/2.,ranges[2][1]+3.*self.spacings[2]/2.,dosignal=dosignal)
...
class Image3DDialog(ImageDialog):
    def __init__(self, parent=None,):
        defaultoptions={"show_contrast":True,"show_xsection":False,"show_ysection":False,"lock_aspect_ratio":False}
        ...
        self.rectangleerasertool.connect(self.rectangleerasertool,SIGNAL('suppress_area()'),self.suppress_rectangular_area) 
        self.image=None
        self.xcurve=None
        self.ycurve=None
        self.zcurve=None
        ...
        QObject.connect(self.imagemasking.run_tool,SIG_VALIDATE_TOOL, self.delete_masked_values)
        
    def register_image_tools(self):
        ImageDialog.register_image_tools(self)
        self.openfiletool=self.add_tool(OpenFileTool)
        self.openfiletool.connect(self.openfiletool,SIGNAL("openfile(QString*)"), self.open_file)
    ...
    def create_menubar(self):       
        ...
        self.menuSlices.addActions((self.action2D,self.actionStraight,self.actionTilted))
        ...
        QObject.connect(self.actionStraight, SIGNAL("triggered()"), self.make_straight_slices) 
        QObject.connect(self.actionTilted, SIGNAL("triggered()"), self.make_tilted_slices) 
        QObject.connect(self.actionGriddata, SIGNAL("triggered()"),self.griddata_interpolation)
        QObject.connect(self.actionLaplace, SIGNAL("triggered()"),self.laplace_interpolation)
        ...
        QObject.connect(self.actionPreferences, SIGNAL("triggered()"),self.set_preferences)
    ...
    def make_default_image(self):
        self.image = make.maskedimagenan(np.zeros((1,1)),interpolation='nearest',xdata=[0,0],ydata=[0,0])
    ...
    def update_image(self):
        ...
        if self.data is not None:
            if self.p_panel.plane.isChecked():
                ...
                self.make_projection(axis,integral=integral,bg=bg) #show integrated intensity for selected range
        ...
        if self.laplaceparam.doslice:
            ...
            for k in range(kmax):
                ...
                if np.sum(cont)>0:
                    ...
                    sle=np.nan_to_num(self.data[:,:,k]/cont)   #normalized values, 0 if no contributions
                    ...
                    while x>0:
                        ...
                        cont[mpoints]=np.where(sle[mpoints]>0,self.laplaceparam.fillc,0)  #replace with default value fillc
            ...
            self.update_image()
            i=QMessageBox.question(self,"what should I do","what should I do","apply","continue","cancel")
            ...
            sle=np.nan_to_num(self.data/slc)   #normalized values, 0 if no contributions
            ...
            while x>0:
                ...
                self.contributions[mpoints]=np.where(sle[mpoints]>0,self.laplaceparam.fillc,0)  #replace with default value fillc
            ...
            while doit:
                ...
                i=QMessageBox.question(self,"what should I do",txt,"apply","continue","cancel")
    ...
    def griddata_interpolation(self):
        ...
        self.contributions[~hascont]=griddata((x[hascont], y[hascont],z[hascont]),  # points we know
                                        self.contributions[hascont],                    # values we know
                                        (x[~hascont], y[~hascont], z[~hascont]),
                                        method='nearest',fill_value=0.)  #fill with 0 for points outside of the convex hull of the input points
        #interpolate normalized intensities and multiply by contributions
        print "interpolate counts"
        self.data[~hascont]=self.contributions[~hascont]*griddata((x[hascont], y[hascont],z[hascont]), # points we know
    ...
    def select_plane(self,axis,bg=False):
        ...
        with np.errstate(invalid='ignore'):
            xmin=self.x_values[1]-self.x_values[3]/2.     #raw table limits
            xmax=self.x_values[2]+self.x_values[3]/2.
            ymin=self.y_values[1]-self.y_values[3]/2.
            ymax=self.y_values[2]+self.y_values[3]/2.
            zmin=self.z_values[1]-self.z_values[3]/2.          
            zmax=self.z_values[2]+self.z_values[3]/2.
            ...
            if axis==0:
                ...
                if self.p_panel.show_contributions.isChecked():
                    ...
                    projx=np.sum(self.data,axis=(1,2))/np.sum(self.contributions,axis=(1,2))
                    projy=np.sum(self.data[ix0,:,:],axis=1)/np.sum(self.contributions[ix0,:,:],axis=1)
                    projz=np.sum(self.data[ix0,:,:],axis=0)/np.sum(self.contributions[ix0,:,:],axis=0)
                self.showndatalimits=[[ix0,ix0+1],[0,self.data.shape[1]],[0,self.data.shape[2]]]   #limits shown
                ...
                if self.p_panel.show_contributions.isChecked():
                    ...
                    projx=np.sum(self.data[:,iy0,:],axis=1)/np.sum(self.contributions[:,iy0,:],axis=1)
                    projy=np.sum(self.data,axis=(0,2))/np.sum(self.contributions,axis=(0,2))
                    projz=np.sum(self.data[:,iy0,:],axis=0)/np.sum(self.contributions[:,iy0,:],axis=0)
                self.showndatalimits=[[0,self.data.shape[0]],[iy0,iy0+1],[0,self.data.shape[2]]]   #limits shown
            else:
                ...
                if self.p_panel.show_contributions.isChecked():
                    ...
                    projx=np.sum(self.data[:,:,iz0],axis=1)/np.sum(self.contributions[:,:,iz0],axis=1)
                    projy=np.sum(self.data[:,:,iz0],axis=0)/np.sum(self.contributions[:,:,iz0],axis=0)
                    projz=np.sum(self.data,axis=(0,1))/np.sum(self.contributions,axis=(0,1))
                self.showndatalimits=[[0,self.data.shape[0]],[0,self.data.shape[1]],[iz0,iz0+1]]   #limits shown
            ...
            # i3,i4,i5,i6 indices for bg subraction
                ...
                if axis==0:
                    bg_value=((np.sum(self.data[i3:i4,:,:],axis=axis)+np.sum(self.data[i5:i6,:,:],axis=axis))
                            /(np.sum(self.contributions[i3:i4,:,:],axis=axis)+np.sum(self.contributions[i5:i6,:,:],axis=axis)))
                elif axis==1:
                    bg_value=((np.sum(self.data[:,i3:i4,:],axis=axis)+np.sum(self.data[:,i5:i6,:],axis=axis))
                            /(np.sum(self.contributions[:,i3:i4,:],axis=axis)+np.sum(self.contributions[:,i5:i6,:],axis=axis)))
                else:
            ...
            ai=self.x_values[1]
            af=self.x_values[2]+self.x_values[3]/2.  #pour eviter les problemes d'arrondi. arange s'arrete tout seul
            da=self.x_values[3]
            xpts=np.arange(ai,af,da)
            ai=self.y_values[1]
            af=self.y_values[2]+self.y_values[3]/2.
            da=self.y_values[3]
            ypts=np.arange(ai,af,da)
            ai=self.z_values[1]
            af=self.z_values[2]+self.z_values[3]/2.
            da=self.z_values[3]
            ...
            if self.p_panel.zlog.isChecked():
                v=np.nanmin(proj[np.array(proj,bool)])  #minimum of the nonzero values of the array
                proj=np.log(proj+v)
            ...
            self.p_panel.xcurve.set_data(xpts[np.isfinite(projx)],projx[np.isfinite(projx)])
            if self.p_panel.autoscale.isChecked():
                self.p_panel.xplot.do_autoscale()
            else:
                self.p_panel.xplot.replot()
            self.p_panel.ycurve.set_data(ypts[np.isfinite(projy)],projy[np.isfinite(projy)])
            
            if self.p_panel.autoscale.isChecked():          
                self.p_panel.yplot.do_autoscale()
            else:
                self.p_panel.yplot.replot()
            self.p_panel.zcurve.set_data(zpts[np.isfinite(projz)],projz[np.isfinite(projz)])
            if self.p_panel.autoscale.isChecked():                    
    ...
    def make_projection(self,axis,integral=False,bg=False):
        ...
        with np.errstate(invalid='ignore'):
            ...
            if self.p_panel.show_contributions.isChecked():
                ...
                proj=np.sum(self.data[ix1:ix2,iy1:iy2,iz1:iz2],axis=axis)/np.sum(self.contributions[ix1:ix2,iy1:iy2,iz1:iz2],axis=axis)
    ...
    def make_2D_slices(self):
        ...
        ix1=int(round((prefs.mins[0]-self.x_values[1])/self.x_values[3]))  #borne inclue[
        ix2=int(round((prefs.maxs[0]-self.x_values[1])/self.x_values[3]))  #borne exclue[
        ...
        for i in range(prefs.stepn):
            ...
            if self.p_panel.swap.isChecked():   #x along 
                fitwindow.add_data(np.transpose(data),x_range,y_range,weights,title=title,xlabel=self.x_label,ylabel=self.y_label,tags=[(tag1,Q)])
            else:
                fitwindow.add_data(data,y_range,x_range,weights,title=title,xlabel=self.y_label,ylabel=self.x_label,tags=[(tag1,Q)])
            j1=j2
    ...
    def make_straight_slices(self):
        ...
        proj3=np.sum(self.data[ix1:ix2,iy1:iy2,iz1:iz2],axis=i3)*prefs.steps[i3]*ni3
        ...
        title=title+"\nslices along "+self.labels[i1]+'\nraw sum along '+self.labels[i3]+' in the range [%g,%g['%(prefs.mins[i3],prefs.maxs[i3])
        
        self.slicewin = CurveDialog(edit=False, toolbar=True, wintitle="CurveDialog test",
                        options=dict(title=title, xlabel=self.labels[i2],
                                     ylabel="intensity"))      
        ...
        for i in range(prefs.stepn):
            ...
            v2=vmin+(i+1)*prefs.stepw
            j2=int(round((v2-mins[i1])/steps[i1]))  #borne [
            proj1=np.sum(proj3[j1:j2],axis=0)  #we do not multiply by stepw in order to have the density along the rod, as usual
            #print 'proj1',proj1.shape
            cont1=np.sum(cont3[j1:j2],axis=0)
            ydata=np.ma.masked_where(cont1==0,proj1)
            ydata=ydata/cont1        #on divise par les contributions donc on a une densite
            xdata=np.ma.masked_where(cont1==0,pts)
            
            Q=mins[i1]+float(j1+j2-1)*steps[i1]/2.
            title=self.labels[i1]+'=%f'%(Q)
            item=make.curve(xdata.compressed(), ydata.compressed(), title=title, color=QColor(red,green,blue))
            ...
            Qs.append(Q)    
            ncoupstot=np.sum(proj1)/(prefs.steps[i3]*ni3)  #nombre de coups detectes
            print Q,ncoupstot
        ...
        self.intwin = CurveDialog(edit=False, toolbar=True, wintitle="CurveDialog test",
                        options=dict(title=title, xlabel=self.labels[i1],
                                     ylabel="intensity"))      
    ...
    def make_tilted_slices(self):
        ...
        #print prefs.stepn,prefs.stepw
        #print prefs.i1,prefs.i2,prefs.i3
        i0,i1,i2=prefs.i0,prefs.i1,prefs.i2    #i0 slice direction,i1 scan direction, i2 raw sum direction
        ...
        title=title+"\nslices along "+self.labels[i2]+'\nraw sum along '+self.labels[i0]
        
        self.slicewin = CurveDialog(edit=False, toolbar=True, wintitle="CurveDialog test",
                        options=dict(title=title, xlabel=self.labels[i0],
                                     ylabel="intensity"))      
        ...
        for istep in range(prefs.stepn):
            ...
            for k in range(k1,k2):
                ...
                #center of the rod
                x=pos1[i0]+(pos2[i0]-pos1[i0])*(z-pos1[i2])/(pos2[i2]-pos1[i2])
                y=pos1[i1]+(pos2[i1]-pos1[i1])*(z-pos1[i2])/(pos2[i2]-pos1[i2])
                ...
                if iy1>=0 and iy2<=data_sli.shape[1]:            
                    scan_values=scan_values+np.sum(data_sli[ix1:ix2,iy1:iy2],axis=0)*(ix2-ix1)*prefs.ranges[0] 
                    scan_conts=scan_conts+np.sum(cont_sli[ix1:ix2,iy1:iy2],axis=0)*(ix2-ix1)*prefs.ranges[0] 
                else:
                    ...
                    scan_values[idec1:idec2]=scan_values[idec1:idec2]+np.sum(data_sli[ix1:ix2,iy1:iy2],axis=0)*(ix2-ix1)*prefs.ranges[0]  
...
#           self.figfit=grafit.Ui_FitWindow()  
    ...
    def set_fit(self):
      self.init_fit()
      QObject.connect(self.p_panel, SIGNAL('active_plot_changed()'), self.update_curve_fit)  
    ...
    def show_fit(self,item):
        if item is not None:
          try:
            x,y=item.get_data()    
            ylabel='intensity'
            self.figfit.setvalexp(x,y,xlabel=item.xlabel,ylabel=ylabel,tags=item.tags,title=item.curveparam.label)
            self.figfit.show()
          except:
...
def test():
    ...
    win.open_file('G:/Documents Geoffroy PREVOT/ANR-Germanene-2017/Manip SOLEIL nov 2019/binoculars depot1 corrige/hdf5/hdf5 in plan 0.1 et 2.5/0.1/Al111_depot1_258-258.hdf5')
