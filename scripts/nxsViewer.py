...
class MeshParameters:
    ...
    def __init__(self,Nx=1,Ny=1,xmin=0.,ymin=0.,xmax=0.,ymax=0.,ix=0,iy=0,iz=0,ilog=0,io=0,angle=0.):
        self.Nx=Nx
        self.Ny=Ny
        self.xmin=xmin
        self.ymin=ymin
        ...
        self.b1=0. #percentile min pour estimation du background
        self.b2=10. #percentile max pour estimation du background
        
class DataFilter:
    # classe pour les parametres de filtre   
    def __init__(self,value=0.,role="none"):
        self.ifil=-1
        self.value=value
        self.role=role
...
class DataSet:
    ...
    def __init__(self,longname,shortname,datafilter=None,pref=None):
            ...
            for leaf in fichier.listNodes('/')[0].scan_data:
                if leaf.shape[0] == self.npts:
                    ...
                    if len(nodelongname) == 0 :
                        nodelongname=leaf.name  # si pas de nom long on garde le nom nxs
                    self.nodelongnames.append(nodelongname)   
                    self.data=np.concatenate((self.data,leaf.read()[1:]))  #on ajoute les donnees au tableau np, on enleve le premier point

                    if pref.inamedisplay<=1:
                        nodenickname=nodelongname.split("/")[-1]   #on prend le dernier
                        self.nodenicknames.append(nodenickname) 
                            
                    elif pref.inamedisplay==2:
                        try:
                            namesplit=nodelongname.split("/")
                            nodenickname=namesplit[-2]+"/"+namesplit[-1]   #on prend les deux derniers si possible
                            self.nodenicknames.append(nodenickname)
                        except:
                        ...
                        self.nodenicknames.append(nodelongname) #on prend le nom long complet
        ...
        if pref.inamedisplay==0:
            for i in range(self.nmotors-1):  # pas la peine de faire le dernier point!
                ...
                if nickname in self.nodenicknames[i+1:]:   #alors item en double
                    ...
                    namesplit=nodelongname.split("/")
                    try:
                        nodenickname=namesplit[-2]+"/"+namesplit[-1]   #on prend les deux derniers
                    except:
                    ...
                    try:
                        while 1:
                            ...
                            self.nodenicknames[j]=nodenickname     #attention, on ne garantit pas que nodenickname!=nickname
        ...
        #print self.data[0]
        test=np.any(self.data!=0,axis=0)  # si une valeur non nulle, la condition est verifiee
        #print test
        ...
        if datafilter is not None:
            ...
            if datafilter.role is not "none" and datafilter.ifil > -1:
                ...
                if datafilter.irole == 1:
                    ...
                    self.data=np.compress(self.data[datafilter.ifil]>datafilter.value,self.data,axis=1)
...
class Set3DparametersWindow(QtGui.QDialog):
    ...
    def __init__(self,dataset,params,NX=1,NY=1,mins=None,maxs=None):
        ...
        super(Set3DparametersWindow, self).__init__()  #permet l'initialisation de la fenetre sans perdre les fonctions associees
        ...
        QtCore.QObject.connect(self.Cancel, QtCore.SIGNAL(_fromUtf8("clicked()")), self.closewin)
        QtCore.QObject.connect(self.OK, QtCore.SIGNAL(_fromUtf8("clicked()")), self.appl)
        QtCore.QObject.connect(self.paramx, QtCore.SIGNAL(_fromUtf8("currentIndexChanged (int)")), self.changex)
        QtCore.QObject.connect(self.paramy, QtCore.SIGNAL(_fromUtf8("currentIndexChanged (int)")), self.changey)
        self.exec_()
...
class SetFilterWindow(QtGui.QDialog):
    ...
    def __init__(self,datafilter):
        ...
        super(SetFilterWindow, self).__init__()  #permet l'initialisation de la fenetre sans perdre les fonctions associees
...
class SelectMultiPointTool(InteractiveTool):
    ...
    def __init__(self, manager, mode="reuse", on_active_item=False,
        ...
        super(SelectMultiPointTool, self).__init__(manager, toolbar_id,
    ...
    def on_active_curve_label(self,x,y,plot):
        ...
        if curve:
            ...
            for i in range(curve.dataset.nmotors):
                label=label+curve.dataset.nodenicknames[i]+":%f\n"%(curve.dataset.data[i,self.impact])
            return label


class Draw3DTool(CommandTool):
    def __init__(self, manager, toolbar_id=DefaultToolbarID):
        super(Draw3DTool,self).__init__(manager, _("Draw a 3D map"),
    ...
    def activate_command(self, plot, checked):
        ...
        itemselection=list(item for item in itemselected if ICurveItemType in item.types())   #on ne prend que les courbes
        ...
        mins=np.amin(np.concatenate(list(item.mins for item in trueselection)).reshape(Nitem,Nmotors),axis=0)
        maxs=np.amax(np.concatenate(list(item.maxs for item in trueselection)).reshape(Nitem,Nmotors),axis=0)
        ...
        Set3DparametersWindow(dataset0,meshpar,NX=item0.dataset.npts,NY=Nitem,mins=mins,maxs=maxs)  #ouvre une fenetre de dialogue
...
class ImageFileList(QtGui.QListWidget):
    ...
    def getnxs(self):
        ...
        self.nxs =  [(name, os.path.join(root, name))
...
def prepare3Dmesh(trueselection,meshpar):
        ...
        if meshpar.bc:
          ...
          zz=np.concatenate(list((item.data[meshpar.iz]-vm[item]) for item in trueselection))  
        ...
        if meshpar.io:
          ...
          #print bornes
          win=make3Dmesh(x2,y2,zz,zlog=meshpar.ilog,bins=bins,bornes=bornes,xtitle=xtitle2,ytitle=ytitle2 ,ztitle=ztitle)
          plot=win.get_plot()
          eps=1.e-10
          if unorm==vnorm:
            if abs(angle-90.)<eps:
             sg='p4'
            elif abs(angle-60.)<eps:
             sg='p3'
            else:
             sg='cm'
          else:
          ...
          make3Dmesh(xx,yy,zz,zlog=meshpar.ilog,bins=bins,bornes=bornes,xtitle=xtitle,ytitle=ytitle,ztitle=ztitle)


def make3Dmesh(x,y,z, zlog=0,bins=None, bornes=None,xtitle="x",ytitle="y",ztitle="3D mesh"):
    ...
    if bins[0]>1:   #s'il y a plus d'une ligne en y on fait une map
      ...
      bornesy[0]=bornesy[0]-deltay/2.
      bornesy[1]=bornesy[1]+deltay/2.
      
    
      if zlog:
          H, yedges, xedges = np.histogram2d(y, x, bins=bins, range=bornes, weights=np.log(z+1)) 
      else:
      ...
      win.set_data(values,xdata=[xedges[0],xedges[-1]],ydata=[yedges[0],yedges[-1]],colormap='jet')
      win.show_image()
      plot=win.get_plot()
      ...
      if zlog:
          H, xedges = np.histogram(x, bins=bins[1], range=bornes[1], weights=np.log(z+1)) 
      else:
      ...
      xedges=xedges-(xedges[1]-xedges[0])/2.   #on decale d'un demi pas
      xedges=xedges[1:]      
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(QtGui.QApplication.translate("MainWindow", "nxsViewer", None, QtGui.QApplication.UnicodeUTF8))
        ...
        for param in range(self.maxpar):
            ...
            QtCore.QObject.connect(cbox, QtCore.SIGNAL(_fromUtf8("stateChanged (int)")), self.boxselection_changed)
        ...
        self.actionNewWindow .setText(QtGui.QApplication.translate("MainWindow", "New window", None, QtGui.QApplication.UnicodeUTF8))
        self.actionNewWindow .setObjectName(_fromUtf8("NewWindow "))
       
        self.actionXLog = QtGui.QAction(MainWindow)
        self.actionXLog.setText(QtGui.QApplication.translate("MainWindow", "X log", None, QtGui.QApplication.UnicodeUTF8))
        self.actionXLog.setObjectName(_fromUtf8("XLog"))
        self.actionXLog.setCheckable(1)
        
        self.actionYLog = QtGui.QAction(MainWindow)
        self.actionYLog.setText(QtGui.QApplication.translate("MainWindow", "Y log", None, QtGui.QApplication.UnicodeUTF8))
        ...
        self.actionFilter.setText(QtGui.QApplication.translate("MainWindow", "Filter policy", None, QtGui.QApplication.UnicodeUTF8))
        self.actionFilter.setObjectName(_fromUtf8("InitFilter "))       

        self.actionInitFit = QtGui.QAction(MainWindow)
        self.actionInitFit.setText(QtGui.QApplication.translate("MainWindow", "Fit", None, QtGui.QApplication.UnicodeUTF8))
        self.actionInitFit.setObjectName(_fromUtf8("InitFit "))       
        
        self.action3DMesh = QtGui.QAction(MainWindow)
        self.action3DMesh.setText(QtGui.QApplication.translate("MainWindow", "3DMesh", None, QtGui.QApplication.UnicodeUTF8))
        ...
        QtCore.QObject.connect(self.actionOpen, QtCore.SIGNAL(_fromUtf8("triggered()")), self.ouvrirFichiers)
        QtCore.QObject.connect(self.actionSave, QtCore.SIGNAL(_fromUtf8("triggered()")), self.enregistrer)
        QtCore.QObject.connect(self.actionQuit, QtCore.SIGNAL(_fromUtf8("triggered()")), self.fermer)

        QtCore.QObject.connect(self.actionXLog, QtCore.SIGNAL(_fromUtf8("triggered()")), self.setXLog)
        QtCore.QObject.connect(self.actionYLog, QtCore.SIGNAL(_fromUtf8("triggered()")), self.setYLog)
        QtCore.QObject.connect(self.actionNewWindow, QtCore.SIGNAL(_fromUtf8("triggered()")), self.makewindow)
        
        QtCore.QObject.connect(self.actionFilter, QtCore.SIGNAL(_fromUtf8("triggered()")), self.setfilter)        
        QtCore.QObject.connect(self.actionInitFit, QtCore.SIGNAL(_fromUtf8("triggered()")), self.initfit)
        QtCore.QObject.connect(self.action3DMesh, QtCore.SIGNAL(_fromUtf8("triggered()")), self.draw3Dmesh)
        QtCore.QObject.connect(self.actionPreferences, QtCore.SIGNAL(_fromUtf8("triggered()")), self.setpreferences) 
        
        QtCore.QObject.connect(self.listfiles, QtCore.SIGNAL(_fromUtf8("itemSelectionChanged ()")),self.item_selectionchanged)
        QtCore.QObject.connect(self.listfiles, QtCore.SIGNAL(_fromUtf8("itemClicked (QListWidgetItem *)")),self.item_clicked)

        QtCore.QMetaObject.connectSlotsByName(MainWindow)
        self.clipboard = QtGui.QApplication.clipboard()

        #self.Image.setFocus()
        self.csuite=("#00bfff","#ff6a6a","#98fb98","#ffd700","#ee82e2","#e9967a","#4169e1","#ff0000","#00ff00","#ffa500")
        self.icolor=0
...
#           self.figfit=grafit.Ui_FitWindow()  
    ...
    def showfit(self,itemselected):
        ...
        try:
            ...
            leafnames=["I14-C-CX2__EX__DIFF-UHV-H__#1/raw_value",
        ...
        self.figfit.setvalexp(x2,y2,xlabel=xlabel,ylabel=ylabel,title=itemselected.curveparam.label,xlog=self.actionXLog.isChecked(),ylog=self.actionYLog.isChecked(),tags=tags,followscans=self.pref.followscans)
        self.figfit.show()
    ...
    def initlist(self):
        self.paramlist=['y','ybrut','filters','mu',' ','delta','gamma','h','k','l','q']
    ...
    def drawcurve(self,dataset):
        ...
        for iy in range(dataset.nmotors):
            if self.YCheckBoxes[iy].isChecked():
                ...
                if title not in self.listtitles:   #on ne retrace pas des courbes deja faites
                    
                    self.lastitem=self.drawgraph(absi,dataset.data[iy],title=title,xlabel=xlabel,ylabel=ylabel)
    ...
    def draw3Dmesh(self):
        ...
        for item in self.listfiles.selectedItems():
            ...
            dataset=DataSet(longname,shortname,datafilter=self.datafilter,pref=self.pref)
        ...
        mins=np.amin(np.concatenate(list(dataset.mins for dataset in trueselection)).reshape(Nitem,Nmotors),axis=0)
        maxs=np.amax(np.concatenate(list(dataset.maxs for dataset in trueselection)).reshape(Nitem,Nmotors),axis=0)
        Set3DparametersWindow(dataset,self.meshpar,NX=dataset.npts,NY=Nitem,mins=mins,maxs=maxs)  #ouvre une fenetre de dialogue
    ...
    def makewindow(self):
        self.win = CurveDialog(edit=False, toolbar=True, wintitle="Scan window",
        ...
        self.win.add_tool(SelectMultiPointTool, title=None, on_active_item=True,
                           mode="create")
    ...
    def addplot(self,*items):
        ...
        if self.actionAutoscale.isChecked(): self.plot.do_autoscale(replot=True)
        self.plot.show_items()
    ...
    def item_selectionchanged(self):    
        ...
        if self.nXBoxesChecked>0 and self.nYBoxesChecked>0:   #pas la peine de perdre du temps si rien n'est coche
    ...
    def item_clicked(self,item):
        ...
        self.currentdataset=DataSet(longname,self.shortname,datafilter=self.datafilter,pref=self.pref)
