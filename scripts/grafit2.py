...
from guiqwt.plot import CurveWidget,CurveDialog,PlotManager,BaseCurveWidget
...
from guidata.qt.QtGui import (QSyntaxHighlighter,QTextCharFormat,QListWidget,QListWidgetItem,QTextEdit,QTableWidget,QTableWidgetItem,
                              QLabel,QMenu,QAction,QCursor,QColor,QIcon,
                              QMessageBox,QInputDialog,QFileDialog,QDialog,QMainWindow,QSplitter,QApplication,
                              QPushButton,QButtonGroup,QSpinBox,QRadioButton,QCheckBox)
from guidata.qtwidgets import DockableWidgetMixin
from guidata.qthelpers import add_actions,get_std_icon

import sys, weakref
from guiqwt.curve import PlotItemList,CurveItem
...
from guiqwt.signals import SIG_END_RECT,SIG_TOOL_JOB_FINISHED, SIG_START_TRACKING, SIG_CLICK_EVENT,SIG_RANGE_CHANGED,SIG_ITEM_REMOVED
from guiqwt.config import _
from guiqwt.panels import ID_ITEMLIST
...
try:
    ...
    "you need to instal lmfit version: 0.7.2"
    
#print "grafit imported"
SHAPE_Z_OFFSET = 1000
...
def lorentz(x,p):
    ...
    return p[0]/(1.+((x-p[1])/0.5/p[2])**2)
    
def lorentzder(x,p):
    #p[0] max d'intensite, p[1] centre, p[2],FWHM
    return p[0]*(p[1]-x)/p[2]/(1.+((x-p[1])/0.5/p[2])**2)**2
...
def gausslor(x,p):
    ...
    if p[3]>0:
      ...
      return (p[0]*np.exp(-((x-p[1])/w/p[2])**2))*(1-b)+(pp0/(x-pp1)**2)*b
    elif p[3]<0:
      w=0.600561204393225
      p32=p[3]*p[2]
      pp1=p32+p[1]-((w*p[2])**2)/p32
...
def skew(x,p):
    w=0.600561204393225
    return p[0]*np.exp(-((x-p[1])/w/p[2])**2)*(1.+erf((x-p[1])*p[3]/w/p[2]))
    
    
def voigt(x,p):
    w=0.600561204393225
    return p[0]*((1.-p[3])*np.exp(-((x-p[1])/w/p[2])**2)+p[3]/(1.+((x-p[1])/0.5/p[2])**2))

def step(x,p):
    w=0.600561204393225
    return 0.5*p[0]*(1.-erf((x-p[1])/w/p[2]))

def door(x,p):
    return p[0]/2.*(erf((x-p[1]+p[2]/2.)/p[3])-erf((x-p[1]-p[2]/2.)/p[3]))

#integrales des fonctions
#integrales des fonctions
def Ilorentz(p):
  return p[0]*p[2]*np.pi/2.
...
def Ivoigt(p):
  wpi=1.064467019431226
  return p[0]*p[2]*((1.-p[3])*wpi+p[3]*np.pi/2.)
...
def Igausslor(p):
  ...
  pp0=p[0]*np.exp(-((p32)/w/p[2])**2)*(p[1]+p32-pp1)**2
  return p[0]*p[2]*wpis2*(1.+erf(p[3]/p[2]))+pp0/pp

#dictionnaire reliant la fonction au nom
curvetypes=dict(gaussian=gauss,lorentzian=lorentz,voigt=voigt,door=door,step=step,gaussian_derivative=gaussder,lorentz_derivative=gaussder,gaussian_lorentzian=gausslor,skew=skew)
#dictionnaire reliant l'integrale au nom
inttypes=dict(gaussian=Igauss,lorentzian=Ilorentz,voigt=Ivoigt,door=None,step=None,gaussian_derivative=None,lorentz_derivative=None,gaussian_lorentzian=Igausslor,skew=Igauss)

curvenames=list(("gaussian","lorentzian","voigt","door","step","gaussian_derivative","lorentz_derivative","gaussian_lorentzian","skew"))
curveparams=dict(gaussian=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0.,None)),
                 lorentzian=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0.,None)),
                 voigt=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0,None),("eta",0.5,0.,1.)),
                 door=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0,None),("steep",0.25,0.,None)),
                 step=(("int",1.,None,None),("pos",0.,None,None),("steep",1.,0.,None)),
                  gaussian_derivative=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0.,None)),
                  lorentzian_derivative=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0.,None)),
                  gaussian_lorentzian=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0.,None),("eps",0.1,None,None)),
                  skew=(("int",1.,None,None),("pos",0.,None,None),("fwhm",1.,0.,None),("alpha",0.,None,None)))
                 
#parametres de reference communs aux differentes courbes
int0=Parameter("int",1.,min=0.)
pos0=Parameter("pos",0.)
fwhm0=Parameter("fwhm",1.,min=0.)
eta0=Parameter("eta",0.5,min=0.,max=1.)
steep0=Parameter("steep",0.25,min=0.)
alpha0=Parameter("alpha",0.)
eps0=Parameter("alpha",0.)
...
class readfilewindow(QDialog):  
    def __init__(self,params,ficlines):
        self.params=params
        super(readfilewindow, self).__init__()  #permet l'initialisation de la fenetre sans perdre les fonctions associees
        ...
        self.connect(self.heading, SIGNAL(_fromUtf8("valueChanged(int)")), self.changeheading)
        ...
        self.QObject.connect(self.delimiters, SIGNAL(_fromUtf8("buttonClicked(int)")),self.setpattern)
...
class FitTable(QTableWidget):
    ...
    def __init__(self,parent=None,extendedparams=None):    
        ...
        self.connect(self.horizontalHeader(), SIGNAL(_fromUtf8("sectionPressed(int)")), self.sectionClicked)
    ...
    def initparams(self,extendedparams=None,settry=False,setopt=False):
        ...
        n=0
        oldrc=self.rowCount()
        newrc=len(self.extendedparams.partry)-len(self.extendedparams.tags)  #on n'affiche pas les tags
    ...
    def initcontexts(self):
        ...
        self.connect(self.setwholeastry, SIGNAL(_fromUtf8("triggered()")), self.set_wholeastry)           
        ...
        self.connect(self.inverserestrains, SIGNAL(_fromUtf8("triggered()")), self.inverse_restrains)           
        ...
        self.connect(self.releaseall, SIGNAL(_fromUtf8("triggered()")), self.release_all)           
    ...
    def set_cv(self,curvetype=None,params=None,bg=0.):
        ...
        if curvetype is None:
            ...
            self.isfitted=False
            cvtyp,ok=QInputDialog.getItem(self,"curve type","curve choice",curvenames,editable=False)
            if ok:
            ...
            if self.activecurve=="new":
                ...
                self.extendedparams.changecurve(paramvalues=params,icurve=self.activecurve)
                print self.activecurve
                #si la courbe est la premiere courbe, on met le fond a la valeur bg
                if self.activecurve is 0:
                  self.extendedparams.partry['pol0'].value=bg 
                elif self.activecurve is -1:
                  icurve=len(self.extendedparams.curveslist)-1
                  if icurve is 0:
                    self.extendedparams.partry['pol0'].value=bg                                    
                self.initparams(settry=True)
    ...
    def drawtry(self):
        ...
        ytry=model(self.extendedparams.partry, xtry, (self.extendedparams.degpol, self.extendedparams.curveslist))
    ...
    def drawopt(self):
        ...
        yopt=model(self.extendedparams.paropt, xopt, (self.extendedparams.degpol, self.extendedparams.curveslist))
    ...
    def save_fit(self):
        ...
        if self.savename is not None:
          ...
          #on ecrit la valeur des integrales brutes
          if self.saveint:
              self.ints=integral(self.extendedparams.paropt,self.extendedparams.curveslist)
...
class ExtendedParams():
    ...
    def __init__(self):
        ...
        self.curveslist=list()  #liste d'un dictionnaire comprenant type de courbe et nom des parametres
    ...
    def addbackground(self):
        ...
        self.partry[entry].number=0   #on rajoute une reference au nombre de la courbe (0 pour le polynome) 
        self.partry[entry].express=''  #on ajoute a la classe une variable 'express' qui est la valeur de l'expression pas forcement appliquee (pour la garder en memoire)
    ...
    def removecurve(self,k):
        ...
        for k2 in range (k,ncurve):
            ...
            for cp in curveparams[curvename]:
                ...
                self.partry.add(entry2, value=self.partry[entry].value,vary=self.partry[entry].vary,min=self.partry[entry].min,max=self.partry[entry].max,expr=self.partry[entry].expr)                
    ...
    def startfit(self):
        ...
        result=minimize(residual, self.partry, args=(self.xexp, self.yexp, (self.degpol, self.curveslist)))
...
class MaskTool(BaseCursorTool):
    ...
    def __init__(self, manager, toolbar_id=DefaultToolbarID,
        ...
        super(MaskTool, self).__init__(manager, toolbar_id, title=title,
                                              icon=icon, tip=tip)
...
class FitTool(CommandTool):
    ...
    def __init__(self, manager, toolbar_id=DefaultToolbarID):
        CommandTool.__init__(self, manager, _("Fit"),icon=gausspath,
    ...
    def create_action_menu(self, manager):
        ...
        self.connect(lorentzaction, SIGNAL(_fromUtf8("triggered()")), self.drawnewlorentz) 
        ...
        self.connect(gaussderaction, SIGNAL(_fromUtf8("triggered()")), self.drawnewgaussder) 

        lorentzderaction = QAction(QIcon(gaussderpath),"Lorentz_derivative",self)
        self.connect(lorentzderaction, SIGNAL(_fromUtf8("triggered()")), self.drawnewlorentzder) 

        gaussloraction = QAction(QIcon(gausslorpath),"Gaussian_lorentzian",self)
        self.connect(gaussloraction, SIGNAL(_fromUtf8("triggered()")), self.drawnewgausslor) 
        ...
        add_actions(menu, (gaussaction,lorentzaction,voigtaction,stepaction,dooraction,gaussderaction,lorentzderaction,gaussloraction,skewaction,resetaction))
        
        self.gausstool=self.manager.add_tool(RectangularActionToolCX,actiongauss, toolbar_id=None, icon="rectangle.png",title = "Gauss")
        self.lorentztool=self.manager.add_tool(RectangularActionToolCX,actionlorentz, toolbar_id=None, icon="rectangle.png",title = "Lorentz")
        self.voigttool=self.manager.add_tool(RectangularActionToolCX,actionvoigt, toolbar_id=None, icon="rectangle.png",title = "Voigt")
        self.steptool=self.manager.add_tool(RectangularActionToolCXY,actionstep, toolbar_id=None, icon="rectangle.png",title = "Step")
        self.doortool=self.manager.add_tool(RectangularActionToolCX,actiondoor, toolbar_id=None, icon="rectangle.png",title = "Door")
        self.gaussdertool=self.manager.add_tool(RectangularActionToolCX,actiongaussder, toolbar_id=None, icon="rectangle.png",title = "GaussDer")
        self.lorentzdertool=self.manager.add_tool(RectangularActionToolCX,actionlorentzder, toolbar_id=None, icon="rectangle.png",title = "LorentzDer")
        self.gausslortool=self.manager.add_tool(RectangularActionToolCX,actiongausslor, toolbar_id=None, icon="rectangle.png",title = "GaussLor")
        self.skewtool=self.manager.add_tool(RectangularActionToolCX,actionskew, toolbar_id=None, icon="rectangle.png",title = "Skew")
...
class RunTool(CommandTool):  
    def __init__(self, manager, toolbar_id=DefaultToolbarID):
        ...
        CommandTool.__init__(self, manager, _("Run"),icon=runpath,
                                            tip=_("Perform fit"),
                                            toolbar_id=toolbar_id)
...
class YFullRangeTool(CommandTool):  
    def __init__(self, manager, toolbar_id=DefaultToolbarID):
        CommandTool.__init__(self, manager, _("Full Range"),icon=yfullrangepath,
...
class PrefTool(CommandTool):  
    ...
    def create_action_menu(self, manager):
        ...
        self.connect(self.saveprefaction, SIGNAL(_fromUtf8("triggered()")), self.savepref) 

        self.updatestartaction = manager.create_action(_("Start with last fit"),
                                                  toggled=self.updatestart)
        self.updatestartaction.setChecked(False)

        self.showtagsaction = QAction("Show tags",self)
        self.connect(self.showtagsaction, SIGNAL(_fromUtf8("triggered()")), self.showtags) 
        ...
        menu = QMenu()
        #add_actions(menu, (self.gausscurve.action,self.lorentzcurve.action,self.voigtcurve.action,self.stepcurve.action,self.doorcurve.action,resetaction))
        add_actions(menu, (self.saveprefaction,self.updatestartaction,self.showtagsaction,self.scaleprefaction))
...
def actiondoor(plot, p0, p1):
    x0try,itry,wtry,btry=get_xiwb_cx(plot,p0,p1)
    plot.tabval.set_cv("door",(itry,x0try,wtry,wtry/4.),bg=btry)  
    
def actiongaussder(plot, p0, p1):
    x0try,itry,wtry,btry=get_xiwb_cx(plot,p0,p1)
    plot.tabval.set_cv("gaussian_derivative",(itry,x0try,wtry),bg=btry+itry/2.)
...
def actionskew(plot, p0, p1):
    x0try,itry,wtry,btry=get_xiwb_cx(plot,p0,p1)
    plot.tabval.set_cv("skew",(itry,x0try,wtry,0.),bg=btry)
...
class Ui_FitWindow(QMainWindow):
    ...
    def setupUi(self):
        ...
        z=np.empty((0,))
        self.empty=True
        self.plot.curveexp=make.curve(z,z,marker="Diamond",markerfacecolor='r',markeredgecolor='r',markersize=4,linestyle="NoPen",title='experiment')
        ...
        self.plot.hide_items((self.plot.curveexp,self.plot.curvetry,self.plot.curveopt))
    ...
    def getfitresult(self):
        return [self.tabval.extendedparams.paropt,integral(self.tabval.extendedparams.paropt,self.tabval.extendedparams.curveslist)]
    ...
    def setvalexp(self,xexp,yexp,xlabel="x",ylabel="y",title="scan",xlog=False,ylog=False,tags=[],followscans=False):
        ...
        if self.tabval.isfitted and self.tabval.issaved is False:
            ...
            i=QMessageBox.question(self, "save", "do you want to save fit?", "Yes","No", "Cancel")
        ...
        if followscans and not self.empty:
          ...
          cen0=(max(self.plot.xexp)+min(self.plot.xexp))/2.
          cen1=(max(xexp)+min(xexp))/2.
          shift=cen1-cen0 
