            ...
            'plot' :
             {
              ...
              "shape/gridshape/symbol/alpha" : 0.,
              ...
              "shape/gridshape/sel_symbol/alpha" : .7,
              ...
              "shape/reconstructionshape/symbol/alpha" : 0.,
              ...
              "shape/reconstructionshape/sel_symbol/alpha" : .7,
...
class SpotShape(PointShape):
    ...
    CLOSED = False
    def __init__(self, x0=0, y0=0,shapeparam=None,spotshapeparam=None,gridname="",grid=None,h=None,k=None):
...
class GridShape(PointCloudShape,PolygonShape):
    ...
    def __init__(self, O=[0,0],u=[100,0],v=[0,100],order=1,sg='p4',axesparam=None, shapeparam=None, gridshapeparam=None):
    ...
    def __getstate__(self):
        return self.shapeparam, self.O, self.u, self.v, self.order, self.sg, self.slaves, self.z()
...
class ReconstructionShape(PointCloudShape,PolygonShape):
    ...
    def __init__(self, A=[2.,0.],B=[0.,2.],master=None,order=2,axesparam=None, shapeparam=None, reconstructionshapeparam=None):
        ...
        if reconstructionshapeparam is None:
            self.reconstructionshapeparam = ReconstructionShapeParam(_("ReconstructionShape"), icon="rectangle.png")
        else:
            self.reconstructionshapeparam = reconstructionshapeparam
            self.reconstructionshapeparam.update_shape(self)


        self.A=np.array(A,float)   #vecteur 1 de la matrice de la reconstruction dans l'espace direct
        self.B=np.array(B,float)   #vecteur 2 de la matrice de la reconstruction dans l'espace direct
    ...
    def set_points(self):
        ...
        self.u=[self.master.u[0]*self.Ar[0]+self.master.v[0]*self.Ar[1],
                self.master.u[1]*self.Ar[0]+self.master.v[1]*self.Ar[1]]
        self.v=[self.master.u[0]*self.Br[0]+self.master.v[0]*self.Br[1],
...
class RectanglePeakTool(RectangularShapeTool):
    ...
    def add_shape_to_plot(self, plot, p0, p1):
        ...
        win=plot.window()
        data=win.image.data    #on recupere les donnes associees a l'image dans l'application maitresse
        #conversion des unites du plot en pixel de l'image
        points=np.empty(plotpoints.shape)
        for i in range(4):
            points[i,0],points[i,1]=win.image.get_pixel_coordinates(plotpoints[i,0],plotpoints[i,1])
...
class DistorsionCorrectionTool(CommandTool):
    ...
    def activate_command(self, plot, checked):
        self.items=list([item for item in plot.get_items() if isinstance(item, SpotShape)])
        xexp=list()
        yexp=list()
        xth=list()
        yth=list()
        ...
        for item in self.items:
            ...
            xexp.append(x)
            yexp.append(y)
            x1,y1=win.image.get_pixel_coordinates(O[0]+h*u[0]+k*v[0],O[1]+h*u[1]+k*v[1])
            xth.append(x1)
            yth.append(y1)
...
class D2DDialog(ImageDialog):
    ...
    def __init__(self, edit=False, toolbar=True,options={"show_contrast":True}, parent=None, data=None):
    ...
    def duplicate(self):
        ...
        for item in griditems:
            ...
            for slave in slaves:
                slave.shapeparam.update_param(slave)#certains parametres ne sont pas forcement actualises
