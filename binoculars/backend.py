        ...
        if self.config.limits is not None:
            for lim in self.config.limits:
                if len(lim) != len(labels):
                    raise errors.ConfigError('dimension mismatch between projection axes ({0}) and limits specification ({1}) in {2}'  # noqa
                                             .format(labels, self.config.limits, self.__class__.__name__))  # noqa
        if ',' in res:
            self.config.resolution = util.parse_tuple(res, type=float)
            if not len(labels) == len(self.config.resolution):
                raise errors.ConfigError('dimension mismatch between projection axes ({0}) and resolution specification ({1}) in {2}'  # noqa
                                         .format(labels, self.config.resolution, self.__class__.__name__))  # noqa
        else:
...
def get_dispatcher(config, main, default=None):
    return _get_backend(config, 'dispatcher', dispatcher.DispatcherBase,
                        default=default, args=[main])
...
def _get_backend(config, section, basecls, default=None, args=[], kwargs={}):
    ...
    if type is None:
        raise errors.ConfigError("required option 'type' not given in section '{0}'"  # noqa
                                 .format(section))
    type = type.strip()
    ...
    if ':' in type:
        try:
            ...
            raise errors.ConfigError("invalid type '{0}' in section '{1}'"
                                     .format(type, section))
        try:
            backend = __import__('backends.{0}'.format(modname),
                                 globals(), locals(), [], 1)
        except ImportError as e:
            raise errors.ConfigError("unable to import module backends.{0}: {1}"  # noqa
                                     .format(modname, e))
    ...
    if clsname in names:
        ...
        if issubclass(cls, basecls):
            ...
            raise errors.ConfigError("type '{0}' not compatible in section '{1}': expected class derived from '{2}', got '{3}'"  # noqa
                                     .format(type, section, basecls.__name__, cls.__name__))  # noqa
    else:
        raise errors.ConfigError("invalid type '{0}' in section '{1}'".format(type, section))  # noqa
